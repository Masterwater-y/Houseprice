  ...
  <el-table
    ...
    <el-table-column label="操作" align="center">
      <template #default="scope">
        <el-button
          size="mini"
          @click="handleSearch(scope.row)">查看</el-button>
        <el-button
          size="mini"
          type="danger"
...
export default {
  ...
  setup(props){
    ...
    const indextypeFormatter=(row,column,cellValue)=>{
      let res=data.indexOptions.filter(val=>(val.label==cellValue))[0].name
      return res
    }
    ...
    const handleDelete=()=>{
      ...
      DeleteFavour({favourid:data.dialogFavourid}).then(response=>{
        ElMessage.success({
          type:'success',
          message:'删除成功'
        })
